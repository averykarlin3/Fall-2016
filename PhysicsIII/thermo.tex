...
\2 It is noted that changing the direction of the cycle would change the sign of the line integral, due to depending on which path direction is traversed for expansion and contraction ($\oint PdV$ for a clockwise curve due to each pathway integral being negative)
...
\3 For some small temperature difference, $K = \frac{-L}{A\Delta T}\frac{\delta Q}{dt}$, due to the temperature gradient and length being
...
\2 Permanent diatomic gases, such as $H_2, O_2, N_2, NO, CO, F_2, Cl_2$, have constant values of $C_V \approx \frac{5}{2}R, C_P \approx \frac{7}{2}R$
...
\1 By Claudius's Theorem, for any two reversible paths making a closed cycle, $\int_{R_1i}^f \frac{\delta Q}{T} + \int_{R_2f}^i \frac{\delta Q}{T} = 0$
\2 Since each path is reversible, $\int_{R_2f}^i \frac{\delta Q}{T} = -\int_{R_2i}^f \frac{\delta Q}{T}$, such that the integral of $\frac{\delta Q}{T}$ for each reversible path with the same initial and final state is equal
