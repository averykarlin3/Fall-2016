...
\1 By Maxwell's equations, the energy density for each field is found such that $u = \frac{1}{2}\epsilon_0E^2 + \frac{1}{2\mu_0}B^2 = \epsilon_0E^2$, such that the magnetic and electric field energies are equal at all points
\2 $S = \epsilon_0E^2c = \frac{EB}{\mu_0}$, such that the Poynting vector is the vector instantaneous energy flow per unit area in the direction of propogation, $\vec{S} = \frac{1}{\mu_0}\vec{E}\times\vec{B}$, with the magnitude as the power per unit area
\2 The intensity is equal to the average of the Poynting vector as a result, such that $I = <S> = \frac{E_{max}B_{max}}{2\mu_0} = \frac{1}{2}\epsilon_0cE_{max}^2$, with total power as $P = \int IA$ assuming the waves are isotropic
\3 In addition, the root-mean-square field value is defined as $E_{rms}\sqrt{2} = E_{max}$, and cooresponding for magnetic fields, such that $I = \frac{E^2_{rms}}{c\mu_0}$
\end{outline*}
...
\2 This allowed general relativity, based on the idea of curved spacetime, rather than Euclidean perpendicular axes, allowing non-inertial reference frames, such as a large/global scale in which gravity applies
...
\2 It is noted that this ignores the lack of rigidity in special relativity, in which case the box would stretch, still shifting the center of mass
...
\1 As a result, for some particle fission of initial mass M into two m masses, $E_f = \frac{2mc^2}{\sqrt{1 - \frac{v^2}{c^2}}}$, such that by conservation of energy, $M = \frac{2m}{\sqrt{1 - \frac{v^2}{c^2}}}$, such that mass is not conserved
\2 Thus, the separate laws of Conservation of Mass and Energy have been modified into the single Conservation of Energy including mass energy law
\2 Due to the low energy in relativistic particle collisions, the electron volt (1 eV = 1.6 $* 10^{-19} J$) is used, equal to the energy gained by an electron accelerating through 1 V voltage
\3 Thus, momentum can be measured in $\frac{eV}{c}$ and mass can be measured in $\frac{eV}{c^2}$ by the $E^2 = p^2c^2 + m^2c^4$ formula, sometimes leaving off the unit denominator as shorthand
\1 In 1905, Einstein stated that light acts in some cases as disrete energy quanta, called photons, with energy $E = hf$, such that the momentum and energy equations are invalid due to 0 mass and infinite $\gamma$
\2 On the other hand, the four-vector magnitude is valid, such that it is found $E = pc$, where p is the magnitude of the momentum
\end{outline*}
\section{Helliwell Chapter 12 - Applications}
\begin{outline*}
\1 The ionization energy of an atom is the energy required to separate, also called the binding energy of the electron and the atom, equal for a hydrogen atom to 13.6 eV
\2 While there is some kinetic energy of thw two particles, it can be considered negligable due to being vastly smaller than ionization or particle energy, such that $E_H + E_{ion} = E_p + E_e$, such that the mass of the original atom is less than that of the particles
\2 As a result, larger binding energy of two particles results in a smaller mass of the combination
\1 Conversely, during the joining of two oxygen atoms, such as during combustion, a photon is released, such that some of the mass energy is lost as kinetic energy of the photon
\1 The nuclear strong force is a strong, short-range force which acts on nucleons (neutrons/protons) close together, creating a far greater binding energy than that of ionization
\2 As atoms get heavier, the bonding energy increases drastically, peaking around Iron (56 protons), then slowly decreasing, due to the mass of protons repelling eachother countering the strong force of all of the nucleons
\2 This allows low mass atoms to be fused easily into an atom with higher binding energy, while high mass atoms are fissioned easily into the same, causing a higher binding energy per nucleon and a high release of kinetic energy as a result
\1 Mass-energy/momentum equations combined with conservation of momentum and energy can be used to calculate the characteristics of particle decay purely with decay masses
\2 It was noted that the energies of electron beta decay from a Carbon-14 atom varied, such that a third neutral particle must be emitted, called a neutrino, such that the energies and velocities have infinite possibilities
\1 Stable particles are often studied by collisions of stable particles into an unstable particle, such as a photon and proton to produce either a neutral pion and proton or positive pion and neutron
\2 The collision into the stationary proton by the photon produces two particles moving in the same direction at different angles from the x-axis, $\theta \geq 0, \phi \leq 0$
\3 As a result, momentum is conserved by component, such that $p_{\gamma} = p_{\pi}cos(\theta) + p_pcos(\phi)$ and $0 = p_{\pi}sin(\theta) - p_psin(\phi)$
\3 The momentum vector triangle can be drawn, such that by the law of cosines, $p_p^2 = p_{\pi}^2 + p_{\gamma}^2 - 2p_{\pi}p_{\gamma}cos(\theta)$
\2 By the energy-momentum equation, energy conservation can be written as $p_{\gamma}c + m_pc^2 = \sqrt{p_{\pi}^2c^2 + m_{\pi}^2c^4} + \sqrt{p_p^2c^2 + m_p^2c^4}$, such that if either $cos(\theta), p_{\pi}$, or $p_p$ are measured, the remaining two can be found by these equations
\1 The Compton experiment fired x-rays at graphite and measured the wavelength of the x-rays scattered by electrons as a function of the angle deflected, displaying particle behavior
\2 By the law of cosines, $p_e^2 = (\frac{hf_i}{c})^2 + (\frac{hf_f}{c})^2 - 2\frac{h^2f_if_fcos(\theta)}{c^2}$, where $p_e$ is the momentum of the recoil electron, related by $p_ec^2 = E_e^2 - m_e^2c^4$, such that $E_e$ is the energy of the electron after recoil
\2 This can then be solved, substituting wavelength into frequency, such that the scattered wavelength is longer, such that energy is lost
\2 Electrons that were not changed were those that reflected off an atom as a whole, rather than an electron, such that the high mass results in infintessimal change in wavelength
\1 Conservation of momentum and energy also imply certain forbidden reactions, such as an electron and position moving with equal but opposite momentum, colliding to produce a photon, which is unable to have zero momentum/energy
\2 Similarly, particles are unable to break into multiple particles with a greater total mass than the original particle by energy conservation
\1 Photon rockets are spaceships sending photons as fuel, making it more efficient due to the high speed
\2 It is found that $\gamma = \frac{1 + \frac{M^2}{M_0^2}}{2\frac{M}{M_0}}$, such that the portion of the ship taken up by fuel would have to be high to achieve a high gamma factor
\2 This could be done by lasers, though these would be heavy and inefficient, or by matter-antimatter annhilation, though this is difficult to produce and store
\end{outline*}
...
Is there a greater gravitational effect -  relativistic mass

$\Delta KE = \int^B_A \vec{F}_{net} \cdot d\vec{r} = ... = \int^B_A \vec{v} \cdot d\vec{p} = \vec{v} \cdot \vec{p} - \int^B_A \vec{p} \cdot d\vec{v} = mv^2\gamma + \frac{mc^2}{\gamma}|^B_A$
If $v_A = 0$, $K = (\gamma - 1)mc^2$, reduces to classical form by binomial expansion, and $E = \gamma m c^2 = KE + mc^2$

$E^2 = \vec{p}^2c^2 + m^2c^4$ - p is relativisitc
$E = hn = pc, p = hn/c = h/lambda$ for photon
\end{document}