...
\3 Thus, if $\delta t = t$, $\psi(x, t) = \psi(x - vt, 0) = \psi(u)$
...
\2 Thus, for a traveling string wave, $v = \pm \sqrt{\frac{W}{M}}$, such that the velocity depends on the tension and density
...
\2 Thus, the two respective solutions are for a positive and negative velocity for a traveling wave, such that it is equal to the solution to a wave with positive and negative velocity
\2 Thus, it is found that $\psi(x, t) = \psi(x \pm vt) = \psi_+(x - |v|t) + \psi_-(x + |v|t)$, taking the specific solution at t = 0
...
\3 By integrating the velocity formula for t = 0, $\psi_-(x) = \psi_+(x) + c$, such that at t = 0, $\psi(x) = 2\psi_+(x) + c = 2\psi_-(x) - c$
